...
useEffect(() => {
    const fetchBookedSlots = async () => {
      ...
      try {
        // 캐시 방지를 위해 타임스탬프 추가
        const response = await fetch(`${GOOGLE_SHEET_URL}?date=${formData.date}&v=${Date.now()}`);
        const data = await response.json();
        
        // 서버 데이터를 "HH:mm" (예: 08:30, 18:30) 형식으로 강제 통일
    ...
    fetchBookedSlots();
  }, [formData.date, GOOGLE_SHEET_URL]);

  // [수정] 시간 슬롯 생성 함수 (변수 정의 오류 해결 버전)
  const getTimeSlots = () => {
    if (!formData.date) return [];
    
    const selectedDate = new Date(formData.date);
    const day = selectedDate.getDay(); // 0:일, 1:월, ...
    if (day === 1) return []; // 월요일 휴무
    
    const slots = [];
    
    // 요일에 따른 영업 시간 설정
    let startHour = (day >= 2 && day <= 5) ? 17 : 12; // 화-금: 17시, 주말: 12시
    let endLimit = (day === 0) ? 22 : 22.5; // 일요일: 22시, 나머지: 22:30

    for (let h = startHour; h <= Math.floor(endLimit); h++) {
      for (let m of ["00", "30"]) {
        // 일요일 22:30 제외 로직
        if (h === 22 && m === "30" && day === 0) break;
        
        // 비교를 위한 표준 시간값 생성 (예: "08:30", "18:00")
        const timeValue = `${h.toString().padStart(2, '0')}:${m}`;
        
        // 실시간 예약 목록에 있는지 확인
        const isReserved = bookedSlots.includes(timeValue);
        ...
        slots.push({ 
          value: timeValue, 
          label: isReserved ? `${displayTime} (Full)` : displayTime,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.privacyAccepted) {
      Swal.fire({
        ...
        customClass: { confirmButton: 'px-10 py-2 rounded-full font-bold text-black' }
      });
      return;
    }
    ...
    try {
      await fetch(GOOGLE_SHEET_URL, {
        ...
        body: JSON.stringify({ ...formData, mode: "new" }),
      });

      Swal.fire({
        title: `<span style="color: white; font-weight: bold;">Request Sent</span>`,
        html: `<span style="color: rgba(255,255,255,0.8);">We have received your request. Please check your email!</span>`,
        icon: 'success',
        iconColor: '#ffffff',
        background: '#000000',
        ...
        customClass: {
          confirmButton: 'px-10 py-2 rounded-full font-bold text-black transition-opacity hover:opacity-80',
          popup: 'rounded-3xl border border-white/20',
        }
      });

      setFormData({ name: "", email: "", phone: "", guests: "", date: "", time: "", note: "", privacyAccepted: false });
      setBookedSlots([]); // 초기화
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-12 items-stretch">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <h2 className="text-3xl font-bold mb-8">{t("contact.reservation")}</h2>

              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-medium mb-2">{t("contact.name")} *</label>
                    <input type="text" name="name" value={formData.name} onChange={handleChange} required className="w-full bg-white/5 border border-white/20 rounded-2xl px-4 py-3 outline-none" placeholder={t("contact.namePlaceholder")} />
                  </div>

                  <div className="grid sm:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-medium mb-2">{t("contact.phoneNumber")} *</label>
                      <input type="tel" name="phone" value={formData.phone} onChange={handleChange} required className="w-full bg-white/5 border border-white/20 rounded-2xl px-4 py-3 outline-none" placeholder={t("contact.phonePlaceholder")} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-2">{t("contact.emailAddress")} *</label>
                      <input type="email" name="email" value={formData.email} onChange={handleChange} required className="w-full bg-white/5 border border-white/20 rounded-2xl px-4 py-3 outline-none" placeholder={t("contact.emailPlaceholder")} />
                    </div>
                  </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-6">
                    <div>
                      ...
                      <select name="guests" value={formData.guests} onChange={handleChange} required className="w-full bg-white/5 border border-white/20 rounded-2xl px-4 py-3 outline-none [&>option]:bg-black">
                    ...
                    <div>
                      ...
                      <input type="date" name="date" min={minDate} value={formData.date} onChange={handleChange} required className="w-full bg-white/5 border border-white/20 rounded-2xl px-4 py-3 outline-none [&::-webkit-calendar-picker-indicator]:invert" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full bg-white/5 border border-white/20 rounded-2xl px-4 py-3 outline-none [&>option]:bg-black disabled:opacity-50"
                    >
                      ...
                      {getTimeSlots().map(slot => (
                        <option 
                          ...
                          className={slot.disabled ? "text-white/20" : "text-white"}
                        >
                          {slot.label}
                        </option>
                      ))}
                  ...
                  <div>
                    ...
                    <textarea name="note" value={formData.note} onChange={handleChange} rows={3} className="w-full bg-white/5 border border-white/20 rounded-2xl px-4 py-3 outline-none resize-none" placeholder="Allergies, special occasions, etc." />
                  </div>

                  <div className="flex items-start gap-3 pt-2">
                    <input type="checkbox" name="privacyAccepted" id="privacyAccepted" checked={formData.privacyAccepted} onChange={handleChange} required className="mt-1.5 w-4 h-4 rounded border-white/20 bg-white/5 accent-white" />
                    <label htmlFor="privacyAccepted" className="text-sm text-white/60 leading-relaxed cursor-pointer select-none">
                      Ich stimme zu, dass meine Angaben zur Kontaktaufnahme und für Rückfragen dauerhaft gespeichert werden. (Privacy Policy) *
                    </label>
                  </div>
                </div>

                <div className="mt-8">
                  <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} type="submit" disabled={isSubmitting} className={`w-full font-bold py-4 rounded-full transition-colors bg-white text-black ${isSubmitting ? "opacity-50" : ""}`}>
